...
def extract_flagged_sales_orders(file_path, output_path, group_column = "Sales Order"):
    ...
    for row in ws.iter_rows():
        if row[0].fill.start_color.rgb == 'FFFFC7CE':   # stop when we reach the light rec color
            break                                       # stop iterating through the rows when we reach the light red color
    ...
    columns_of_interest = ["Sales Order", "Item ID", "Required Qty"] # HERE is where to modify the columns in the output
    ...
    sorted_df = filtered_df.sort_values(by="Sales Order", ascending=True).reset_index(drop=True)

    # Add empty rows between groups
    grouped = sorted_df.groupby(group_column, group_keys=True).apply(lambda x: x._append(pd.Series(), ignore_index=True)).reset_index(drop=True)
...
def index():
    ...
    if request.method == 'POST':
        ...
        if file:
            ...
            output_path = os.path.join(app.config['UPLOAD_FOLDER'], "processed_output.xlsx")
            extract_flagged_sales_orders(filename, output_path, selected_column)

            return send_from_directory(app.config['UPLOAD_FOLDER'], "processed_output.xlsx", as_attachment=True)
